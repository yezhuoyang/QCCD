...
try {
  ...
  for (const step of spec.steps) {
    ...
    try {
      ...
      else if (step.eval) { Object.assign(rec, await tab.eval(step.eval)); }
      else if (step.wait) {
        const t0 = Date.now(); let last;
        while (Date.now() - t0 < (step.timeout || 15000)) {
          last = await tab.eval(step.wait);
          if (last.value) break;
          await sleep(150);
        }
  ...
  for (const [k, t] of Object.entries(tabs)) out.logs[k] = t.logs.slice(-40);
