...
GATES: list[tuple[str, str, str, str, str]] = [
    ...
    ("h", "h q[0];", "h", "a Clifford rotation: one frame update and one beam, u3(π/2, 0, π)", "chain4"),
    ("x", "x q[0];", "x", "a bit flip: u3(π, 0, π), one beam", "race8"),
    ("y", "y q[0];", "y", "u3(π, π/2, π/2), one beam", "grid2x3"),
    ("z", "z q[0];", "z", "a phase flip: u3(0, 0, π), so the frame update carries the gate and the emitted beam has angle 0", "ring6d"),
    ("s", "s q[0];", "s", "u3(0, 0, π/2): the frame update carries the gate, the beam has angle 0", "chain4"),
    ("t", "t q[0];", "t", "non-Clifford: u3(0, 0, π/4); R10's semantics half uses the exact unitary here", "race8"),
    ("rx", "rx(0.5) q[0];", "rx(θ)", "an arbitrary angle about x: u3(θ, −π/2, π/2)", "grid2x3"),
    ("ry", "ry(0.5) q[0];", "ry(θ)", "u3(θ, 0, 0): one beam", "ring6d"),
    ("rz", "rz(0.7854) q[0];", "rz(θ)", "u3(0, 0, θ): the frame update carries the angle, the beam has angle 0", "chain4"),
    ("cx", "cx q[0],q[1];", "cx", "the entangler: transport to co-locate, then R, MS, R, R, R (the proved decomposition)", "grid2x3"),
    ("cz", "cz q[0],q[1];", "cz", "h · cx · h on the target, so two extra single-qubit pulses around the same MS", "race8"),
    ("swap", "swap q[0],q[1];", "swap", "three CNOTs, so three MS pulses at one site", "ring6d"),
    ("measure", "measure q[0] -> c[0];", "measure", "readout in a zone with SPAM; no pulses, so no gate witness", "chain4"),
    ("reset", "reset q[0];", "reset", "back to |0>: same zone requirement, no witness", "race8"),
]
