...
{{photo:nist_ion_trap_chip.jpg|A surface-electrode ion trap in its mount at NIST|A surface-electrode ion trap in its vacuum mount. The square gold chip at the centre carries the electrodes; two beryllium ions float about 40 µm above it, 40 µm apart, and the copper enclosure and gold mesh around it keep stray charge away from them.|Photo: Y. Colombe/NIST, public domain.}}
...
Inside the well the ion oscillates like a mass on a spring at a **secular frequency** ω of a few megahertz; for a trap of characteristic size `r₀` the RF drive sets it through the Mathieu parameter `q ≈ 2·Q·V_rf / (m·r₀²·Ω²)`, and in the usual regime `ω ≈ q·Ω / (2√2)`. Quantum mechanically that oscillator has energy levels spaced by `ħω`, and its state is described by the mean number of quanta it holds, written **n̄**. An ion fresh from Doppler cooling has n̄ around ten; resolved-sideband cooling brings it below 0.1, close to the motional ground state. Almost every cost on this website is measured in these quanta, so n̄ is the number to keep in mind.
...
How strongly the light couples spin to motion is the **Lamb–Dicke parameter** `η = k·√(ħ / (2·m·ω))`, the ratio of the ion's ground-state wavepacket to the laser wavelength, and the gate works cleanly only in the Lamb–Dicke regime `η²·(2n̄ + 1) ≪ 1`. That inequality is the physics behind the two consequences that shape the whole design. The ions must share a mode, so they must be in the same trap, and a trap where the beams can reach is a **gate zone**; rule [R6b](../rules/#R6b) says a two-qubit gate acts only on ions co-located in one. And the motion must be cold: the gate's error grows with the number of quanta in the mode, because a hot mode blurs the spin-dependent displacement. Quantinuum's H2 processor measured a two-qubit infidelity of 1.84e-3 with cold ions, and the document carries that as the gate's error at n̄ = 0.

## Motion as the currency

Every operation on an ion trap is paid for in motional quanta, and the two ways of paying back are the two kinds of **cooling**. Doppler cooling scatters photons from a laser tuned just below an atomic resonance; the ion absorbs photons preferentially when moving towards the beam, and each scattering event removes a little momentum. It stops at the Doppler limit, where the recoil of the scattered photons balances the cooling, at roughly `n̄ ≈ Γ / (2ω)` quanta for a transition of linewidth Γ. It is fast and works on the whole trap at once from sheet beams, which is why the document's **cool** primitive is a single 300 µs operation that cools every ion in the machine and does not have to be scheduled per ion. Sideband cooling goes further, to the ground state, one quantum at a time, down to about `(Γ / 2ω)²`.
...
{{photo:nist_xjunction_trap.png|A multi-zone surface-electrode trap with an X-junction at NIST|A multi-zone surface trap with an X-junction. The two dark bands are the RF electrodes and the RF null runs between them; the fingers along them are the DC electrodes that shape the wells. Ions are loaded in one zone, transported along the rail and through the junction, and worked on in the experiment zone.|Picture: NIST, public domain.}}

{{photo:nist_racetrack_trap.jpg|NIST's racetrack ion trap|A QCCD layout in gold on quartz: NIST's racetrack trap, with about 150 zones for storing, transporting and probing ions in the central ring and the six channels that leave it. The fanned lines are the leads to the DC electrodes, one per electrode, which is the wiring problem the control-system section is about.|Photo: J. Amini/NIST, public domain.}}

In the studio a device is a map. A **site** is a trapping well; a **segment** is the stretch of RF null an ion is shuttled along between sites; a **junction** is a place where three or more trap axes meet; a **transport loop** is a closed path along which a whole ring of ions can be rotated in step; and a **zone type** is the label a site carries saying how many ions fit there and whether a gate, a measurement or cooling can happen. The diagrams below are drawn from the device data itself, for the small devices the examples on this website run on: a straight register with no junctions, a racetrack whose bends cost nothing, a lattice with real T-junctions beside plain corners, two loops joined at one coupling, and a ring with dock spurs.

{{gallery:chain4,race8,grid2x3,dual3,ring6d}}

Reading the physics above back into the map: a gate needs ions co-located at a gate zone, a shuttle is a well walked along a segment, and everything that moves an ion heats it. The same drawing scales to the devices on the leaderboard.

Not every graph is a surface trap, and three rules say which are. A junction is a place where RF nulls meet, and on a planar chip at most four can meet without their RF electrodes running into each other, so rule [R19](../rules/#R19) bounds the degree of every node (four by default; a document for a three-dimensional trap may declare six). Two rails leaving one node need room between them for their electrodes, so rule [R20](../rules/#R20) requires at least 60 degrees between any two rails at a node, measured from the node positions; a symmetric X-junction has 90. And rails live on one plane, so rule [R21](../rules/#R21) requires that a rail meets only the nodes it ends at and that two rails cross only at a node they share. These three read the drawing alone, before any programme runs. They are why a Tanner graph of a code laid out as rails is not a device, and why a torus drawn on a plane, with its wrap wires running back across the die, is not either; the cylinder on the leaderboard is the planar drawing of the same torus, and it passes.

{{device:ring24_8|The 24-site ring with 8 dock spurs that the landing page's example runs on, drawn from its device data: the loop sites in yellow hold the data ions, the spur ends in blue are the traps where gates happen, and the dark-ringed loop sites are the degree-3 nodes where a spur meets the loop, each a junction the ions pay to cross. The BB code's 144-site ring with 24 spurs on the leaderboard is this drawing six times longer.}}

## Transport primitives, and what each one costs

Transport is done by walking the DC voltages. The elementary moves are the **primitives**, and each is a curve of measured points trading speed against motional excitation: run it faster and the ion picks up more quanta. The physics of that trade is the driven oscillator: a well moved with acceleration `a(t)` drives the ion at its secular frequency, and the quanta left behind are the Fourier component of the acceleration at ω, `Δn̄ = (m / (2·ħ·ω)) · ‖∫ a(t)·e^{iωt} dt‖²`, so a slower, smoother waveform can leave almost nothing behind while a fast one leaves a lot. The table below is the default document's `qccdsim_jones` operating point, which comes from Jones and Murali's cost table; the `transport_excitation` table from Munoz and colleagues offers a slower, colder point for several of them, and the studio lets you choose which point a design runs at.
...
{{run:shuttle|A shuttle through the T-junction of the 2×3 lattice, then a global cool, running on its own page. Press Play or step it; the Report pane prices every step, and the junction crossing shows up as the 3 quanta the walk brings to T0_1h before the cool takes them away.}}
...
{{run:rotate|Broadcast wiring at its best: the outer trap loop of the dual-loop device is filled and rotated past the inner data loop three times. One instruction, one waveform, and every ion on the loop moves together, which is the rigid rotation the leaderboard's fastest designs are built on.}}

Rule [R22](../rules/#R22) states the consequence exactly: one transport cycle is one waveform, so every ion that moves in a cycle performs the same motion, hop by hop, and a site may only opt out. A rigid rotation of a loop is one motion by construction, and so is a batch of docks that all dock the same way; a lattice router that sends one ion east while another goes north in the same cycle is asking for two waveforms, and the verifier now refuses it. The compiler now reserves one motion per cycle while it routes, which costs a lattice design between two and four times its transport time (the BB grids went from about 200 ms to about 400 ms a round) and costs a ring nothing, and that difference is much of why the rings lead the leaderboard.
...
**Anomalous heating.** An ion warms up even when nothing moves it. Electric-field noise from the electrode surfaces, of a magnitude far above what thermal Johnson noise predicts and hence called anomalous, drives the oscillator at its secular frequency. The heating rate is `ṅ = S_E(ω)·e² / (4·m·ħ·ω)` where `S_E` is the spectral density of the field noise; it scales roughly as the inverse fourth power of the ion's distance from the electrodes, `S_E ∝ d⁻⁴`, and drops by about two orders of magnitude when the trap is cooled to cryogenic temperature. The document carries 0.05 quanta per millisecond, and rule [R17](../rules/#R17) accrues it against elapsed time for every ion, moving or not, the gate's own duration included.

**Heating by transport.** Every primitive deposits its quanta into the ions it moves, as in the table above. The document keeps five heating channels separate: shuttling, junction crossings, split and merge, the gate's own duration, and the anomalous heating above. Rule [R15](../rules/#R15) notes that consecutive transports interfere: two excitations with a phase θ between them compose as `n̄ = n̄₁ + n̄₂ + 2·√(n̄₁·n̄₂)·cos θ`, so a phase-aware schedule can cancel excitation, and the additive sum the model uses is the `cos θ = 1` upper bound.
...
**Dephasing while idle.** A qubit that waits for its turn loses coherence: its off-diagonal element decays as `e^(−t / T_coh)`, an error of about `t / (2·T_coh)` for a short wait. The model charges each qubit its share of the round's wall-clock time against the 600 s in the document, which is small for a single round and is why running time, not dephasing, is the number the leaderboard ranks by.
...
{{run:budget|The replay at work on the four-site register: d0 walks three segments to d1, a cool zeroes both, the gate fires and both are measured. Open the Report pane on the page for the quanta by channel, the wall clock by activity, and the gate error read off n̄ at gate time.}}
...
| quantity | formula | where it enters |
|---|---|---|
| pseudopotential | `Ψ = q²·‖E_rf‖² / (4·m·Ω²)` | the trap itself; the [electrode layer](phys.md) solves the fields |
| Mathieu parameter, secular frequency | `q ≈ 2·Q·V_rf / (m·r₀²·Ω²)`, `ω ≈ q·Ω / (2√2)` | the stiffness of a well, hence the mode every price is counted in |
| motional quanta | `E = ħω·(n̄ + ½)` | n̄, the state variable of the replay |
| Lamb–Dicke parameter | `η = k·√(ħ / (2·m·ω))`, regime `η²·(2n̄+1) ≪ 1` | why the MS gate needs cold, co-located ions ([R6b](../rules/#R6b), [R7](../rules/#R7)) |
| Doppler and sideband limits | `n̄_D ≈ Γ / (2ω)`, `n̄_sb ≈ (Γ / 2ω)²` | the cool primitive: 300 µs, removes all |
| transport excitation | `Δn̄ = (m / (2ħω)) · ‖∫ a(t)·e^{iωt} dt‖²` | the (µs, quanta) curve of every primitive; the operating point |
| composition of excitations | `n̄ = n̄₁ + n̄₂ + 2√(n̄₁n̄₂)·cos θ` | [R15](../rules/#R15): the replay takes cos θ = 1, an upper bound |
| anomalous heating | `ṅ = S_E(ω)·e² / (4·m·ħ·ω)`, `S_E ∝ d⁻⁴` | [R17](../rules/#R17): 0.05 quanta/ms, accrued with time |
| junction crossing | 100 µs, 3 quanta on entry to a degree ≥ 3 node | [R18](../rules/#R18): a bend is not a junction |
| ion loss | about 85 uncooled junction round trips | the cooling pass's optional cap |
| gate error | `ε(n̄, N) = ε₀ + k·n̄ (+ chain term ∝ N / ln N)`, `ε₀ = 1.84e-3`, `k = 2e-3` | [R16](../rules/#R16), and the G1 calibration |
| gate budget | `n̄ ≤ 1.0` for both ions at gate time | [R7](../rules/#R7), [R7c](../rules/#R7c) |
| idle dephasing | `e^(−t / T_coh)`, error `≈ t / (2·T_coh)` | the `T_coh` term, 600 s in the document |
| the budget | `−ln F ≈ Σ ε(n̄, N) + n·T / T_coh + Σ ε_SPAM` | the report, the leaderboard's fidelity column |
| below threshold | `p_L ∝ (p / p_th)^((d+1)/2)`, `p_th ≈ 0.7%` for the gross code | why the budget is compared with the threshold |
| junction degree | `deg(node) ≤ 4` on a planar chip | [R19](../rules/#R19): the drawing is a surface trap |
| rail angle | `θ ≥ 60°` between any two rails at a node | [R20](../rules/#R20): room for both rails' electrodes |
| planar rails | a rail meets only its own nodes; rails cross only at a shared node | [R21](../rules/#R21): one plane, no wires across the die |
| one waveform | every moving ion in a cycle has one motion signature | [R22](../rules/#R22): broadcast control, strictly |
